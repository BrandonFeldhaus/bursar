...
Bill and goal rows wrap the checkbox and name in a `<label class="recent-item__toggle">`, so on mobile the whole left side of the 44px row is the tap target. The Goals tab's per-goal amount is `goalFundingTotal(goal, allocations, period.bills)` from `lib/goalFunding.ts` — what ticking applies in that period; the goal editor shows its per-paycheck average.
...
| `goalFunding.ts` | How goals are funded (see *Goal funding*): `sourceAmount`, `goalFundingTotal`, `fundingCandidates`, `fundingAmountLabel`, `goalFundingSources`, `paychecksToGo` |
| `useIsMobile.ts` | `useIsMobile(maxWidth = 600)` — `useSyncExternalStore` over `matchMedia`: the first client render already has the real value (no desktop frame on a phone); the server snapshot is `false` |

### Goal funding (`lib/goalFunding.ts`)
A goal is funded by budget categories and bills (`linkedBudgetCategoryIds` / `linkedExpenseIds` — unchanged schema). Ticking the goal on a period card applies the sum of those sources for that period: a category contributes its allocation (`computeAllocations(period.leftover, categories)`), a bill contributes its amount only in the period it is due, and an id that no longer exists contributes 0. `sourceAmount` is the one place that is computed; `goalFundingTotal` sums a goal's links for one period (Overview). The goals page never uses a single period — a bill not due in it would read $0. `fundingCandidates(state, monthKey)` lists every category and bill with `perPaycheck`: its `sourceAmount` averaged over every paycheck period of the 12 months from `monthKey` (a whole year, so each bill lands exactly 12 times and a monthly bill is spread across 24 / 26 / 52 paychecks); bills also carry `bill: { amount, cadence }`. `fundingAmountLabel` is what the picker and chips show — a bill as billed (`$1,300.00/mo`, `/yr`), a category's `perPaycheck`. `goalFundingSources` filters candidates to the goal's links in order (chips, badges), the editor's "≈ $X per paycheck" is the sum of those sources' `perPaycheck`, and `paychecksToGo(remaining, perPaycheck)` is `ceil` or `null`. The goals page computes candidates once from `currentMonthKey()` and passes them down.

### Add-form pattern (FormDialog)
Every "add a row" flow uses one shared form component rendered inside `FormDialog` (`components/FormDialog.tsx`): a centered `.dialog.dialog--form` on desktop, `BottomSheet` on mobile (`useIsMobile()` at 600px). Both are built on Base UI (`@base-ui/react`): the desktop panel is `Dialog`, `BottomSheet` is `Drawer` with `swipeDirection="down"`. Base UI portals them to `<body>` (a `position: fixed` overlay under an ancestor with `transform`, `filter` or `backdrop-filter` is sized to that ancestor instead of the screen — the bottom nav's `backdrop-filter` did exactly that to the More sheet), traps focus, locks page scroll, closes on Escape / backdrop press / swipe down, and returns focus to the `+ Add X` button on close. `modalInitialFocus` (exported from `BottomSheet.tsx`) picks the element focused on open: `[data-autofocus]`, else the first field in the body (then its first button / link), or the panel itself on touch screens — Base UI's own default would be the header's close button. Open and close both animate: CSS transitions on Base UI's `[data-starting-style]` / `[data-ending-style]`, so drive `open` and keep the component mounted (`<BottomSheet open={x}>`, never `{x && <BottomSheet open>}`) or the close transition is skipped. Nothing scrolls the page. The `+ Add X` header buttons (`.btn.btn--add`) are visible at every width. Add handlers return `boolean` so the dialog closes only on a successful add — validation failures keep it open. Each form file also exports a `xFromDraft(draft, errs)` builder so pages and the Overview share one add path. Forms take `inSheet` (adds `.inline-form--sheet`: no sunk chrome, two columns, one at ≤600px).
...
Other shared pieces: `ImportLedgerButton` (label + hidden file input; unwraps `{ data }` / `{ budgetAppV1 }` / bare state, runs `normalizeParsed`, saves, reloads — used by Settings and the Overview empty state), `OverflowMenu` (⋯ button with a `role=menu` list), `Hint`, `DemoBanner`, `FundingPicker`, `Form` / `advanceOnEnter` (see *Keyboard*).
...
- **Funded by** — explainer line, the goal's sources as removable `.funding-chip`s (name + `fundingAmountLabel`), a `.funding-total` line ("≈ $X per paycheck", plus "about N paychecks to go" while something remains; hidden when the total is 0), and `FundingPicker`. The picker button ("Add funding source") opens the candidates grouped under *Budget categories* and *Bills* as two `<details>`; the goal's type decides which group is open first (debt → Bills, savings → categories). Desktop renders it through a portal as a `position: fixed` `.funding-picker__popover` placed with `--top` / `--left` from the button's rect (re-placed on scroll/resize, closed on outside click / Escape / *Done*); mobile uses `BottomSheet` with 44px rows. Rows toggle: sources already on the goal are checked, and choosing one again removes it. Nothing linkable → a note instead of the button.
...
### Keyboard
There is no custom on-screen-keyboard handling: `useSheetKeyboard` was removed with the move to Base UI's Drawer, pending a device test on iOS Safari. If a sheet misbehaves under the keyboard, try Base UI's `Drawer.VirtualKeyboardProvider` (it scrolls the focused field into view and sets `--drawer-keyboard-inset`; always read it with a `0px` fallback) before writing any `visualViewport` code. Nothing autofocuses a field on touch screens (`modalInitialFocus` focuses the panel).

**Return key** (`components/Form.tsx`): every add form (`AddSourceForm`, `AddBillForm`, `AddCategoryForm`, `AddGoalForm`, the goal editor's adjustment form) is a `<Form className onSubmit>` — a real `<form noValidate autoComplete="off">` whose one `onKeyDown={advanceOnEnter}` moves Enter to the next input / select in DOM order, synchronously (iOS drops the keyboard if focus moves later). On the last field Enter only blurs (closes the keyboard): **Enter never submits** — only the `type="submit"` button does, by design. The auto-saving table edit rows can't be forms, so each `<tr>` takes `onKeyDown={advanceOnEnter}` and behaves the same. Inputs carry `enterKeyHint="next"`, and `"done"` on the last one (computed when the last field depends on cadence); selects and date inputs open pickers, not keyboards, so they get none. Never add a per-input Enter handler.
...
tests/
  ...
  goal-funding.test.ts       # sourceAmount / goalFundingTotal (category, bill, mix, missing id → 0),
                             # fundingCandidates year averages (semi-monthly, biweekly, annual), fundingAmountLabel,
                             # goalFundingSources, paychecksToGo
...
| `--drawer-swipe-movement-y` / `--drawer-swipe-progress` / `--drawer-swipe-strength` | Base UI `Drawer` (on the popup / backdrop, not a TSX `style`) | `.bottom-sheet` (`transform`, close duration) / `.dialog-overlay[role="presentation"]` (`opacity`, close duration) |
...
| `.dialog--form`, `.dialog__head` | `FormDialog`'s desktop panel (Base UI `Dialog.Popup`): the bottom sheet's inner panel centered (`min(100%, 600px)`); fades and scales from 0.96 on open and close |
| `.dialog-overlay[role="presentation"]`, `.dialog-viewport(--sheet)` | Base UI layers under `FormDialog` / `BottomSheet`: the Backdrop reuses `.dialog-overlay` (fades; `position: absolute` on iOS so it covers the area under the browser chrome — needs `body { position: relative }`), the Viewport centers the dialog or (`--sheet`) pins the sheet to the bottom. `ConfirmDialog` still uses a plain `.dialog-overlay` (`role="dialog"`) |
...
| `.bottom-sheet` | Mobile slide-up drawer (`components/BottomSheet.tsx`, Base UI `Drawer.Popup`, portalled to `<body>`); pairs with `.dialog-viewport--sheet`; `FormDialog`, the goal editor, the funding picker and the nav's More use it via `useIsMobile()` at 600px. Slides on `[data-starting-style]` / `[data-ending-style]`, follows `--drawer-swipe-movement-y` while dragged; `--sheet-bleed` runs the paper past the bottom edge so an upward drag shows no gap |
...
- **Mobile keyboard** — never autofocus a text field in a sheet on touch screens, and don't size a sheet with `vh` to dodge the keyboard (see *Keyboard*).
- **Modals portal to `<body>`** — `BottomSheet` and `FormDialog` render through Base UI's `Portal`; never render a `position: fixed` overlay as a plain descendant of the bottom nav, a `.sheet` or anything else with `transform` / `filter` / `backdrop-filter`.
- **Modals stack** — a `BottomSheet` rendered inside another one is a nested Base UI drawer: Escape closes only the top one, the parent gets `data-nested-drawer-open`, and only the outer backdrop renders. That is how the funding picker's sheet opens over the goal editor's sheet.
- **Keep modals mounted** — pass `open={x}`; unmounting on close skips the exit transition. When the content depends on something that goes `null` on close, keep the last value around (the goals page's `sheetGoalId`).
