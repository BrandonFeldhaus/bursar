...
| `useModal.ts` | `useModal(open, onClose, panelRef)` — shared modal behaviour: body scroll lock plus a `touchmove` guard (a drag on the overlay that isn't scrolling something inside it, or placing the caret in the focused field, is cancelled — iOS ignores `overflow: hidden`), Escape closes the topmost modal only, first field focused on open (on `pointer: coarse` the panel itself, `tabIndex={-1}`, so the keyboard doesn't open over an unseen form), focus returned to the opener on close and the page's scroll position restored (unless a link in the modal navigated) |
...
Every "add a row" flow uses one shared form component rendered inside `FormDialog` (`components/FormDialog.tsx`): a centered `.dialog.dialog--form` on desktop, `BottomSheet` on mobile (`useIsMobile()` at 600px). Both containers `createPortal` their overlay to `document.body`: a `position: fixed` overlay rendered under an ancestor with `transform`, `filter` or `backdrop-filter` is sized to that ancestor instead of the screen (the bottom nav's `backdrop-filter` did exactly that to the More sheet). Both containers use `useModal`: Escape and overlay click close them, the first field is focused on open (the panel on touch screens), and focus returns to the `+ Add X` button on close. Nothing scrolls the page. The `+ Add X` header buttons (`.btn.btn--add`) are visible at every width. Add handlers return `boolean` so the dialog closes only on a successful add — validation failures keep it open. Each form file also exports a `xFromDraft(draft, errs)` builder so pages and the Overview share one add path. Forms take `inSheet` (adds `.inline-form--sheet`: no sunk chrome, two columns, one at ≤600px).
...
Mobile browsers don't shrink the layout for the keyboard, so a fixed bottom sheet ends up under it, and the browser then *pans* the view (`visualViewport.offsetTop`) to reveal the tapped field, dragging fixed content off the top of the screen. The browser decides that pan from where the field is right after the focus event, so `BottomSheet`'s `useSheetKeyboard` acts **synchronously in `focusin`** on a field (input / select / textarea): it adds `.bottom-sheet--keyboard` (the sheet pins full height 12px from the top and stays pinned until it closes) and scrolls the focused `.field` inside `.bottom-sheet__body` into the part of the screen the keyboard won't cover — the tallest keyboard measured so far for this layout height (module-level `measured`), or the top 40% until one has been. With the field already in view the browser has no reason to pan. Then it follows `visualViewport` resize / scroll: `--sheet-inset` (layout height − pan − visual height, ignored under 120px) is kept clear at the bottom; while a field keeps focus, whatever height the keyboard gives back (hidden for a select or date picker, shrunk to a number pad) becomes `--sheet-reserve` scroll room at the end of the body, so the scroll position never clamps and the form doesn't move; and if a pan happens anyway, `--sheet-top` moves the sheet down by it so it stays on screen. A sheet with no fields (More, the funding picker) never pins. A new layout height (rotation) drops the reserve; pinch zoom (`scale > 1`) is ignored. Nothing autofocuses a field on touch screens.
...
tests/
  ...
  sheet-keyboard.test.ts     # nextSheetKeyboard: pins on field focus only, inset follows the keyboard and the pan, reserve held while a field is focused, rotation
...
| `--sheet-top` / `--sheet-inset` / `--sheet-reserve` | `useSheetKeyboard` (`style.setProperty` on the panel, not a TSX `style`) | `.bottom-sheet--keyboard` (`inset` top / `padding-bottom`) / its `.bottom-sheet__body` (`padding-bottom`) |
...
| `.bottom-sheet` | Mobile slide-up drawer (`components/BottomSheet.tsx`, portalled to `<body>`); pairs with `.dialog-overlay--sheet`; `FormDialog`, the goal editor, the funding picker and the nav's More use it via `useIsMobile()` at 600px. `--keyboard` (added by `useSheetKeyboard` when a field is tapped) pins it full height above the on-screen keyboard |
...
- **Mobile keyboard** — never autofocus a text field in a sheet on touch screens, and don't size a sheet with `vh` to dodge the keyboard; `useSheetKeyboard` owns that, and its `focusin` work must stay synchronous (a `requestAnimationFrame` there is too late for the browser's reveal-the-field pan). The `touchmove` guard in `useModal` needs its listener on the overlay with `passive: false` (React's `onTouchMove` is passive).
- **Modals portal to `<body>`** — `BottomSheet` and `FormDialog` use `createPortal`; never render a `position: fixed` overlay as a plain descendant of the bottom nav, a `.sheet` or anything else with `transform` / `filter` / `backdrop-filter`.
